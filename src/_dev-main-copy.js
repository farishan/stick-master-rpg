(function(){
    ...
    // Dev Variables
    var _logger = true;
    var _initialPage = 'grove';
    
    var _enemyLive = false;
    var _tutorial = false;
    var _musicVolume = 0.7;
    var _soundVolume = 0.2;
    var _winCondition = 1000;
    var _maxHp = 100;
    var _killEnemyReward = 150;
    var _tameEnemyReward = 100;
    
    var _maxStickCondition = 15;

    var app = new Vue({
        el: '#app',
        data: () => initialData(),
        mounted: function(){
            if(_logger) console.log('[app] Mounted.')
        ...
        methods: {
            ...
            moveEnemies: function() {
                if(_enemyLive){
                    ...
                    if(this.enemy2) this.enemy2.live();
                }
            },
            randomChar: function(){
            ...
            moveListener: function(code, key){
                ...
                if(okay){
                    ...
                    if(this.enemy2) checkSituation(this.char, this.enemy2);
                }
            },
            renderMap(){
                ...
                for (var i = 0; i < this.map.length; i++) {
                    ...
                    if(this.enemy2) this.setCoordinate(this.enemy2, tile);
                }
            },
        ...
        watch: {
            ...
            howtoplayPopup: function(val){
                if(val){
                    this.enemy.stop();
                    if(this.enemy1) this.enemy1.stop();
                    if(this.enemy2) this.enemy2.stop();
                    disableMove();
                }else{
                    this.enemy.live();
                    if(this.enemy1) this.enemy1.live();
            ...
            isCrafting: function(val){
                ...
                console.log(val)
                console.log(this.craftingSlot.length);
            },
            ...
            page: function(to, from){
                if(_logger) console.log('[PAGE] from: '+from, 'to: '+to)
            }
        },
        filters: {
            format: function(val){
    ...
    function Char(data){
        ...
        this.hit = function(target){
            if(_logger) console.log('[HITTING]: ',target)
            if(target){
                var self = this;
        ...
        this.hitEnemy = function(target){
            ...
            function stopping(){
                if(target){
                    ...
                    if(target.name){
                        app.logger('You killed a '+target.name+' !!! (+ '+_killEnemyReward+' exp)');
                    }
                    ...
                    self.health+=20;
                    console.log(self.exp, _killEnemyReward, self.exp + _killEnemyReward)
                    self.exp+=_killEnemyReward;
                }
            }
        };
        ...
        this.tame = function(target){
            if(target.observed){
                ...
                if(roll>=(100-this.damage)){
                    ...
                    app.enemyTamed = true;
                    this.health+=20;
                    this.exp+=_tameEnemyReward;
                    app.logger(target.name + 'has been tamed! (+ '+_tameEnemyReward+' exp)');
                    clearInterval(target.attackInterval);
                    if(this.exp>=_winCondition){
                        gameOver(true);
                    }
    ...
    function initialData(){
        return {
            ...
            environment: new Environment({
                ...
                items: [
                    new Stick({id: 'twg', name: 'Twig', condition: _maxStickCondition-11, accuracy: 100, spawn_percentage: 70, custom: false}),
                    new Stick({id: 'twg', name: 'Sprig', condition: _maxStickCondition-9, accuracy: 100, spawn_percentage: 70, custom: false}),
                    new Stick({id: 'brc', name: 'Branch', condition: _maxStickCondition-5, accuracy: 50, spawn_percentage: 40, custom: false}),
                    new Stick({id: 'lmb', name: 'Limb', condition: _maxStickCondition-1, accuracy: 25, spawn_percentage: 10, custom: false}),
                    new Stick({id: 'bgh', name: 'Bough', condition: _maxStickCondition, accuracy: 25, spawn_percentage: 10, custom: false}),
                ]
            }),
            map: [],
    ...
    function gameOver(condition){
        if(!app.gameIsOver){
            ...
            if(app.enemy1) clearInterval(app.enemy1.moveInterval);
            if(app.enemy1) clearInterval(app.enemy1.attackInterval);
            if(app.enemy2) clearInterval(app.enemy2.moveInterval);
            if(app.enemy2) clearInterval(app.enemy2.attackInterval);
            disableMove();
